...
public static class SpeechSttService
{
    ...
    )
    {
        ...
        Logger.Starting("Speech.Transcribe");

        var path = FileHelpers.ResolvePath(filePath);
        FileHelpers.ReadChecked(
            path,
            Constants.SpeechMaxBytes,
            "Speech"
        );
        var wavPath = path;
        ...
        Logger.ApiRequest("Speech", "StartContinuousRecognition", language);
        ...
        if (ext is not ".wav")
            throw new Exception("Extension is not WAV!");

        Logger.Complete("Speech.Transcribe");
