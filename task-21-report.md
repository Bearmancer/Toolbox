...
### 2. P4.1.2 — Editorconfig violations are build errors

**Method:** Temporary probe file `src/Core/ViolationProbe.cs` with a deliberate `IDE1006` naming violation (`int BadLocal = 1;` — PascalCase local). Built, confirmed failure, deleted probe, rebuilt clean.
...
**Result:** PASS — deliberate style violation fails the build; probe removed, working tree clean of the violation.

### 3. P4.1.3 — Deferred formatting nit in `SacdConvertCommand`
...
### 4. P4.1.4 — No test packages / new dependencies in Phases 1–3
