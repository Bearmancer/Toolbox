...
public record OperationFailed(string Operation, double ElapsedMs, Exception? Cause = null)
    : ILogEvent
{
    ...
    /// <inheritdoc />
    Exception? ILogEvent.Exception => Cause;
}
...
) : ILogEvent
{
    ...
    /// <summary>
    /// Captures the exception's message, type name, and the live object so
    /// Serilog can write the full stack trace to Seq's Exception slot.
    /// </summary>
    public static ErrorOccurred From(Exception ex, string? context = null) =>
        new(ex.Message, context, ex.GetType().Name, Cause: ex);
}

public record FatalOccurred(string Message, string? ExceptionType = null, Exception? Cause = null)
    : ILogEvent
{
    ...
    public static FatalOccurred From(Exception ex) =>
        new(ex.Message, ex.GetType().Name, Cause: ex);
}
