...
public class LastFmSyncOrchestrator(LastFmService service)
{
	...
	)
	{
		...
		Telemetry.Verbose("LastFm sync started (fetchAfter={Since})", fetchAfter?.ToString("yyyy-MM-dd HH:mm") ?? "null");
		...
		loadSw.Stop();
		Telemetry.Verbose("Loaded {Count} existing scrobbles in {ElapsedMs}ms", existing.Count, loadSw.ElapsedMilliseconds);
		...
		if (fetchAfter is { } since)
		{
			...
			if (removedCount > 0)
				Telemetry.Verbose("Removed {Count} scrobbles newer than {Since}", removedCount, since.ToString("yyyy-MM-dd HH:mm"));
		}
		...
		{
			syncSw.Stop();
			Telemetry.Info("Last.fm sync: 0 new, {Removed} removed, {Total} total in {ElapsedMs}ms", removedCount, existing.Count, syncSw.ElapsedMilliseconds);
		...
		mergeSw.Stop();
		Telemetry.Verbose("Merged {New} new + {Existing} existing = {Total} in {ElapsedMs}ms", newScrobbles.Count, existing.Count, merged.Count, mergeSw.ElapsedMilliseconds);

		var saveSw = System.Diagnostics.Stopwatch.StartNew();
		await LastFmState.SaveScrobblesAsync(StateDir, merged);
		saveSw.Stop();
		Telemetry.Verbose("Saved {Count} scrobbles in {ElapsedMs}ms", merged.Count, saveSw.ElapsedMilliseconds);
