...
public class YouTubeDuplicateMerger(YouTubePlaylistService playlistService)
{
    ...
    )
    {
        ...
        IReadOnlyList<DuplicatePlaylistGroup> groups =
            YouTubeDuplicateMergePolicy.FindGroups(allCurrentPlaylists);
        ...
        foreach (DuplicatePlaylistGroup group in groups)
        {
            ...
            var winnerItems = await FetchItemsSafeAsync(
                winner.PlaylistId,
                winner.Title,
                ct
            );
            if (winnerItems is null)
            {
                groupsDeferred++;
                Telemetry.Warn(
                    "Deferred group '{Key}': failed to list winner items",
                    group.Key
                );
                continue;
            }
            ...
            foreach (PlaylistSnapshot loser in losers)
            {
                ...
                if (candidates.MissingVideoIds.Count > 0)
                {
                    ...
                    foreach (var videoId in candidates.MissingVideoIds)
                    {
                        ...
                        ErrorOr<string> insertResult = await playlistService.InsertPlaylistItemAsync(
                            winner.PlaylistId,
                            videoId,
                            ct
                        );
    ...
    )
    {
        var manifest = new MergeManifestRecord
        {
            ...
            Losers = [.. losers.Select(l => new LoserRecord
            {
