# Review package: dd28089..ca7573e

## Commits
ca7573e fix(audio): P1.4 capture per-track split errors with reasons

## Files changed
 .superpowers/sdd/new-mega-plan/task-9-report.md | 117 ++++++++++++++++++++++++
 src/Services/Audio/DsdConvertService.cs         |  27 ++++--
 2 files changed, 136 insertions(+), 8 deletions(-)

## Diff
diff --git a/.superpowers/sdd/new-mega-plan/task-9-report.md b/.superpowers/sdd/new-mega-plan/task-9-report.md
new file mode 100644
index 0000000..9e31f24
--- /dev/null
+++ b/.superpowers/sdd/new-mega-plan/task-9-report.md
@@ -0,0 +1,117 @@
+# Task 9 Report ΓÇö P1.4 Split error capture
+
+**Branch:** sacd-completion-v2 | **Base HEAD:** dd28089 | **File:** `src/Services/Audio/DsdConvertService.cs`
+
+## Subtask 1 ΓÇö Capture split failures into `Dictionary<int,string>` keyed by track number
+
...
+```
+
+**Evidence (source/static):** `splitErrors` declared before the loop, keyed by `track.TrackNumber`. On `splitResult.IsError` the reason (`splitResult.Errors[0].Description`, which for sox carries `sox split exit code {code}: {stderr}` ΓÇö stderr preserved, not discarded) is stored and a `Telemetry.Warn` fires naming track number, output path, and error text. `continue` still advances to the next track, so remaining tracks are attempted.
+
+**Result: PASS** (source/static).
+
+## Subtask 2 ΓÇö Aggregate error names missing track numbers plus per-track reasons
+
+**Diff:**
+```diff
...
+```
+
+**Evidence (source/static):** Every loop iteration either adds to `outputFiles` (split success) or to `splitErrors` (split failure), so `splitErrors.Keys` is exactly the set of missing track numbers ΓÇö the aggregate still names them, now sorted ascending. The description appends per-track reasons (`track N: <reason>`), each reason including sox stderr. `Errors.Audio.ConversionFailed(file, reason)` unchanged; only the reason string is enriched.
+
+**Result: PASS** (source/static).
+
+## Subtask 3 ΓÇö Cancellation behavior preserved
+
+**Evidence (source/static):** The change only handles the returned `splitResult.IsError` case; it adds no `catch`. `sox.SplitTrackAsync` ΓåÆ `ProcessRunner.RunAsync` propagates `OperationCanceledException` on cancellation, which flows up through `ConvertAndSplitAsync` unchanged. Cancellation is not converted into a split failure or stored in `splitErrors`.
+
+**Result: PASS** (source/static).
+
+## Subtask 4 ΓÇö Build
...
+**Result: PASS** (build).
+
+## Subtask 5 ΓÇö Runtime acceptance (injected failure on track 7 of 19)
+
...
+| 5. Runtime acceptance | BLOCKED ΓåÆ P3.4 harness |
+
+**Concerns:** Runtime acceptance remains **BLOCKED** pending the P3.4 durable split harness. The acceptance case ΓÇö injected failure on track 7 of 19 produces a Warn naming track 7 and stderr, aggregate error carries the missing list and per-track reasons, remaining tracks still attempted ΓÇö is **not** runtime-verified here. Source/static checks and the build PASS; the runtime outcome is owned by **P3.4**. New log key `DsdConvert.SplitFailed`; aggregate description format `Incomplete conversion: missing tracks {n}. track {n}: {reason}; ΓÇª`. No public model, `Errors.cs`, or unrelated formatting changed.
diff --git a/src/Services/Audio/DsdConvertService.cs b/src/Services/Audio/DsdConvertService.cs
index a5ab922..39d4a27 100644
--- a/src/Services/Audio/DsdConvertService.cs
+++ b/src/Services/Audio/DsdConvertService.cs
@@ -222,61 +222,72 @@ public sealed class DsdConvertService(
 				probe.SampleRate,
 				probe.Channels,
 				null,
 				ct
